...
class AccountRemissionSend(models.TransientModel):
    _name = 'account.remission.send'
    _inherits = {'mail.compose.message':'composer_id'}
    _description = 'Account Remission Send'
    ...
    def default_get(self, fields):
        ...
        res.update({
            'remission_ids': res_ids,
            'composer_id': composer.id,
        })
        return res

    @api.onchange('remission_ids')
    def _compute_composition_mode(self):
        for wizard in self:
            wizard.composer_id.composition_mode = 'comment' if len(wizard.remission_ids) == 1 else 'mass_mail'
    ...
    @api.onchange('is_email')
    def _compute_remission_without_email(self):
        for wizard in self:
            if wizard.is_email and len(wizard.remission_ids) > 1:
                remissions = self.env['account.remision'].search([
                    ('id', 'in', self.env.context.get('active_ids')),
                    ('partner_id.email', '=', False)
                ])
                if remissions:
                    wizard.remission_without_email = "%s\n%s" % (
                        _("The following invoice(s) will not be sent by email, because the customers don't have email address."),
                        "\n".join([i.name for i in remissions])
                        )
                else:
                    wizard.remission_without_email = False
            else:
                wizard.remission_without_email = False
    ...
    def _send_email(self):
        if self.is_email:
            ...
            if self.env.context.get('mark_remission_as_sent'):
                #Salesman send posted invoice, without the right to write
                #but they should have the right to change this flag
                self.mapped('remission_ids').sudo().write({'is_remission_sent': True})
            for invoice in self.remission_ids:
    ...
    def _print_document(self):
        ...
        self.ensure_one()
        action = self.remission_ids.action_remission_print()
    ...
    def save_as_template(self):
        ...
        action.update({'name': _('Send Remission')})
        return action
