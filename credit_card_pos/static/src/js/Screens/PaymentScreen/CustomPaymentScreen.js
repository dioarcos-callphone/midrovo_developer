odoo.define("credit_card_pos.CustomPaymentScreen", (require) => {
    ...
    const CustomPaymentScreen = (PaymentScreen) => class extends PaymentScreen {
        ...
        setup() {
            super.setup();
            useListener("popup-opened", this.numberBufferDeactivate);
            useListener("popup-closed", this.numberBufferActivate);
        }
        ...
        async addNewPaymentLine({ detail: paymentMethod }) {
            ...
            if (isCard) {
                ...
                const { confirmed, payload: selectedCreditCard } = await this.showPopup(
                    "SelectionPopup",
                    // Usamos el popup correcto para selección de lista
                    {
